		...
		return this.each(function () {
			...
			var sortkeys = ["shiftKey", "altKey", "ctrlKey"], IntNum = function (val, defval) {
				...
				var sep = "<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>", pgid = $(ts.p.pager).attr("id") || "pager", pginp = "", pgl = "<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>", str = "", pgcnt, lft, cent, rgt, twd, tdw, i, clearVals = function (onpaging) {
					/*
					if((ts.p.page).search("^\\d+$")!=0){
						alert('输入错误,请重新输入!');
						return ;
					}
					if(parseInt(ts.p.page)<parseInt(1)){
						alert('输入错误,请重新输入!');
						return ;
					}
					if(parseInt(ts.p.page)>parseInt(ts.p.lastpage)){
				...
				if (ts.p.pginput === true) {
					pginp = "<td dir='" + dir + "'>" + $.jgrid.format(ts.p.pgtext || "", "<input class='ui-pg-input' type='text' id='jqgrid_input' size='2' maxlength='7' value='0' role='textbox'/>", "<span id='sp_1'></span>") + "</td>";
				}
				if (ts.p.pgbuttons === true) {
			...
			$(grid.bDiv).append($("<div style=\"position:relative;>" + (isMSIE && $.browser.version < 8 ? "height:0.01_25;" : "") + "\"</div>").append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({height:ts.p.height + (isNaN(ts.p.height) ? "" : "px"), width:(grid.width) + "px"}).scroll(grid.scrollGrid);
...
(function (a) {
	a.jgrid.extend({getColProp:function (d) {
		...
		b = a.extend({autosearch:true, searchOnEnter:false, beforeSearch:null, afterSearch:null, beforeClear:null, afterClear:null, searchurl:"", stringResult:false, groupOp:"AND"}, b || {});
		return this.each(function () {
			var g = this;
			var c = function () {
				var p = {}, o = 0, u, w, m = {};
				a.each(g.p.colModel, function (v, z) {
					w = this.index || this.name;
					var y = (this.searchoptions && this.searchoptions.sopt) ? this.searchoptions.sopt[0] : "eq";
					switch (this.stype) {
					  case "select":
						u = a("select[name=" + w + "]", g.grid.hDiv).val();
						if (u) {
							p[w] = u;
							m[w] = y;
							o++;
						...
						break;
					  case "text":
						u = a("input[name=" + w + "]", g.grid.hDiv).val();
						if (u) {
							p[w] = u;
							m[w] = y;
							o++;
				...
				var s = o > 0 ? true : false;
				if (!b.stringResult) {
					a.extend(g.p.postData, p);
				} else {
					var t = "{\"groupOp\":\"" + b.groupOp + "\",\"rules\":[";
					var r = 0;
					a.each(p, function (v, x) {
						if (r > 0) {
							t += ",";
						}
						t += "{\"field\":\"" + v + "\",";
					...
					t += "]}";
					a.extend(g.p.postData, {filters:t});
				}
				var q;
				if (g.p.searchurl) {
					q = g.p.url;
				...
				if (a.isFunction(b.beforeSearch)) {
					n = b.beforeSearch.call(g);
				}
				if (!n) {
					a(g).jqGrid("setGridParam", {search:s}).trigger("reloadGrid", [{page:1}]);
				}
				if (q) {
					a(g).jqGrid("setGridParam", {url:q});
				}
			...
			var j = function (m) {
				var o = {}, u, n = 0, w;
				m = (typeof m != "boolean") ? true : m;
				a.each(g.p.colModel, function (v, z) {
					u = (this.searchoptions && this.searchoptions.defaultValue) ? this.searchoptions.defaultValue : "";
					w = this.index || this.name;
					switch (this.stype) {
					  case "select":
						var y;
						a("select[name=" + w + "] option", g.grid.hDiv).each(function (A) {
							if (A == 0) {
								this.selected = true;
							}
							if (a(this).text() == u) {
						...
						break;
					  case "text":
						a("input[name=" + w + "]", g.grid.hDiv).val(u);
						if (u) {
							o[w] = u;
							n++;
						} else {
				...
				if (!b.stringResult) {
					...
					s += "]}";
					a.extend(g.p.postData, {filters:s});
				}
				var p;
				if (g.p.searchurl) {
					p = g.p.url;
				...
				if (a.isFunction(b.beforeClear)) {
					t = b.beforeClear.call(g);
				}
				if (!t) {
					if (m) {
						a(g).jqGrid("setGridParam", {search:r}).trigger("reloadGrid", [{page:1}]);
					}
				}
				if (p) {
					a(g).jqGrid("setGridParam", {url:p});
				}
			...
			var l = function () {
				var m = a("tr.ui-search-toolbar", g.grid.hDiv);
			...
			function f(m, o) {
				...
				if (n[0] != null) {
					jQuery.each(o, function () {
						if (this.data != null) {
							n.bind(this.type, this.data, this.fn);
						} else {
			...
			var k;
			a.each(g.p.colModel, function (u, q) {
				var w = this;
				...
				if (this.search) {
					switch (this.stype) {
					  case "select":
						var m = this.surl || e.dataUrl;
						if (m) {
							var z = i;
							a.ajax(a.extend({url:m, dataType:"html", complete:function (A, n) {
								if (e.buildSelect != null) {
									var B = e.buildSelect(A);
									if (B) {
										a(z).append(B);
									}
								} else {
									a(z).append(A.responseText);
								}
								if (e.defaultValue) {
									a("select", z).val(e.defaultValue);
								}
								a("select", z).attr({name:w.index || w.name, id:"gs_" + w.name});
								if (e.attr) {
									a("select", z).attr(e.attr);
								}
								a("select", z).css({width:"100%"});
								if (e.dataInit != null) {
									e.dataInit(a("select", z)[0]);
								}
								if (e.dataEvents != null) {
									f(a("select", z)[0], e.dataEvents);
								}
								if (b.autosearch === true) {
									a("select", z).change(function (C) {
										c();
										return false;
									});
								}
							...
							var o;
							if (w.searchoptions && w.searchoptions.value) {
								o = w.searchoptions.value;
							} else {
								if (w.editoptions && w.editoptions.value) {
									o = w.editoptions.value;
								}
							}
							if (o) {
								var s = document.createElement("select");
								s.style.width = "100%";
								a(s).attr({name:w.index || w.name, id:"gs_" + w.name});
								if (typeof o === "string") {
									var p = o.split(";"), y, r;
						...
						break;
					  case "text":
						var v = e.defaultValue ? e.defaultValue : "";
						a(i).append("<input type='text' style='width:95%;padding:0px;' name='" + (w.index || w.name) + "' id='gs_" + w.name + "' value='" + v + "'/>");
						if (e.attr) {
						...
						if (b.autosearch === true) {
							if (b.searchOnEnter) {
								a("input", i).keypress(function (A) {
									var n = A.charCode ? A.charCode : A.keyCode ? A.keyCode : 0;
									if (n == 13) {
										c();
										return false;
									}
	...
	b.jgrid.extend({searchGrid:function (c) {
		...
		return this.each(function () {
			...
			if (b("#" + i.themodal).html() != null) {
				...
				M[0].rp = 0;
				b(A).append(M);
				M = b("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='" + (J * 2) + "'>" + a.topinfo + "</td></tr>");
				M[0].rp = 0;
				b(A).append(M);
				if (N) {
					N(b("#" + y));
		...
		return this.each(function () {
			...
			function n(L, R, P, X) {
				var H, K, S, ab, F, V = 0, aa, ac, Z = [], Q = false, N = "<td class='CaptionTD form-view-label ui-widget-content' width='" + d.labelswidth + "'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>", O = "", I = "<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>", M = ["integer", "number", "currency"], U = 0, T = 0, J, G, Y;
				for (var W = 1; W <= X; W++) {
					O += W == 1 ? N : I;
				}
				...
				b(R.p.colModel).each(function (af) {
					H = this.name;
					G = false;
					if (this.editrules && this.editrules.edithidden === true) {
						K = false;
					...
					ac = K ? "style='display:none'" : "";
					Y = (typeof this.viewable != "boolean") ? true : this.viewable;
					if (H !== "cb" && H !== "subgrid" && H !== "rn" && Y) {
						if (Q === false) {
							aa = "";
						} else {
							if (H == R.p.ExpandColumn && R.p.treeGrid === true) {
								aa = b("td:eq(" + af + ")", R.rows[Q]).text();
							} else {
								aa = b("td:eq(" + af + ")", R.rows[Q]).html();
							}
						}
						G = this.align === "right" && J !== 0 ? true : false;
						var ae = b.extend({}, this.editoptions || {}, {id:H, name:H}), aj = b.extend({}, {rowabove:false, rowcontent:""}, this.formoptions || {}), ag = parseInt(aj.rowpos) || V + 1, ai = parseInt((parseInt(aj.colpos) || 1) * 2);
						if (aj.rowabove) {
							var ah = b("<tr><td class='contentinfo' colspan='" + (X * 2) + "'>" + aj.rowcontent + "</td></tr>");
							b(P).append(ah);
							ah[0].rp = ag;
						}
						S = b(P).find("tr[rowpos=" + ag + "]");
						if (S.length == 0) {
							S = b("<tr " + ac + " rowpos='" + ag + "'></tr>").addClass("FormData").attr("id", "trv_" + H);
							b(S).append(O);
							b(P).append(S);
							S[0].rp = ag;
						}
						b("td:eq(" + (ai - 2) + ")", S[0]).html("<b>" + (typeof aj.label === "undefined" ? R.p.colNames[af] : aj.label) + "</b>");
						b("td:eq(" + (ai - 1) + ")", S[0]).append("<span>" + aa + "</span>").attr("id", "v_" + H);
						if (G) {
							b("td:eq(" + (ai - 1) + ") span", S[0]).css({"text-align":"right", width:J + "px"});
						}
						Z[V] = af;
						V++;
					}
				...
				if (V > 0) {
					var ad = b("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='" + (X * 2 - 1) + "' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='" + L + "'/></td></tr>");
					ad[0].rp = V + 99;
					b(P).append(ad);
				}
				return Z;
...
(function (a) {
	a.jgrid.extend({editRow:function (c, i, h, j, b, e, d, f, g) {
		return this.each(function () {
			...
			if (l == "0" && !a(k).hasClass("not-editable-row")) {
				...
				a("td", k).each(function (w) {
					...
					if (v) {
						...
						try {
							o = a.unformat(this, {colModel:p[w]}, w);
						}
						catch (t) {
							o = a(this).html();
						}
		...
		return this.each(function () {
			...
			g = a(j).jqGrid("getInd", c, true);
			if (g == false) {
				return;
			}
			for (var f = 0; f < j.p.savedRow.length; f++) {
				if (j.p.savedRow[f].id == c) {
					d = f;
					break;
				}
			}
			if (d >= 0) {
				...
				a.each(j.p.colModel, function (e, k) {
					if (this.editable == true && this.name in j.p.savedRow[d]) {
						i[this.name] = j.p.savedRow[d][this.name];
					}
				});
				a(j).jqGrid("setRowData", c, i);
				a(g).attr("editable", "0").unbind("keydown");
				j.p.savedRow.splice(d, 1);
			}
			if (a.isFunction(b)) {
				b(c);
			}
...
(function (a) {
	a.jgrid.extend({editCell:function (d, c, b) {
		return this.each(function () {
			var l = this, e, h, k;
			if (!l.grid || l.p.cellEdit !== true) {
				return;
			}
			c = parseInt(c, 10);
			l.p.selrow = l.rows[d].id;
			if (!l.p.knv) {
				a(l).jqGrid("GridNav");
			}
			if (l.p.savedRow.length > 0) {
				if (b === true) {
					if (d == l.p.iRow && c == l.p.iCol) {
						return;
					}
				}
				a(l).jqGrid("saveCell", l.p.savedRow[0].id, l.p.savedRow[0].ic);
			} else {
				window.setTimeout(function () {
					a("#" + l.p.knv).attr("tabindex", "-1").focus();
				}, 0);
			}
			e = l.p.colModel[c].name;
			if (e == "subgrid" || e == "cb" || e == "rn") {
				return;
			}
			k = a("td:eq(" + c + ")", l.rows[d]);
			if (l.p.colModel[c].editable === true && b === true && !k.hasClass("not-editable-cell")) {
				if (parseInt(l.p.iCol) >= 0 && parseInt(l.p.iRow) >= 0) {
					a("td:eq(" + l.p.iCol + ")", l.rows[l.p.iRow]).removeClass("edit-cell ui-state-highlight");
					a(l.rows[l.p.iRow]).removeClass("selected-row ui-state-hover");
				}
				a(k).addClass("edit-cell ui-state-highlight");
				a(l.rows[d]).addClass("selected-row ui-state-hover");
				try {
					h = a.unformat(k, {colModel:l.p.colModel[c]}, c);
				}
				catch (f) {
					h = a(k).html();
				}
				if (!l.p.colModel[c].edittype) {
					l.p.colModel[c].edittype = "text";
				}
				l.p.savedRow.push({id:d, ic:c, name:e, v:h});
				if (a.isFunction(l.p.formatCell)) {
					var i = l.p.formatCell(l.rows[d].id, e, h, d, c);
					if (i != undefined) {
						h = i;
					}
				}
				var g = a.extend({}, l.p.colModel[c].editoptions || {}, {id:d + "_" + e, name:e});
				var j = createEl(l.p.colModel[c].edittype, g, h, true, a.extend({}, a.jgrid.ajaxOptions, l.p.ajaxSelectOptions || {}));
				if (a.isFunction(l.p.beforeEditCell)) {
					l.p.beforeEditCell(l.rows[d].id, e, h, d, c);
				}
				a(k).html("").append(j).attr("tabindex", "0");
				window.setTimeout(function () {
					a(j).focus();
				}, 0);
				a("input, select, textarea", k).bind("keydown", function (m) {
					if (m.keyCode === 27) {
						if (a("input.hasDatepicker", k).length > 0) {
							if (a(".ui-datepicker").is(":hidden")) {
								a(l).jqGrid("restoreCell", d, c);
							} else {
					...
					if (m.keyCode === 13) {
						a(l).jqGrid("saveCell", d, c);
					}
					if (m.keyCode == 9) {
						if (m.shiftKey) {
							a(l).jqGrid("prevCell", d, c);
						} else {
							a(l).jqGrid("nextCell", d, c);
						}
					}
					m.stopPropagation();
				});
				if (a.isFunction(l.p.afterEditCell)) {
					l.p.afterEditCell(l.rows[d].id, e, h, d, c);
				}
			} else {
				if (parseInt(l.p.iCol) >= 0 && parseInt(l.p.iRow) >= 0) {
					a("td:eq(" + l.p.iCol + ")", l.rows[l.p.iRow]).removeClass("edit-cell ui-state-highlight");
				...
				k.addClass("edit-cell ui-state-highlight");
				a(l.rows[d]).addClass("selected-row ui-state-hover");
				if (a.isFunction(l.p.onSelectCell)) {
					h = k.html().replace(/\&#160\;/ig, "");
					l.p.onSelectCell(l.rows[d].id, e, h, d, c);
				}
			}
			l.p.iCol = c;
			l.p.iRow = d;
...
(function (a) {
	a.jgrid.extend({setColumns:function (b) {
		...
		return this.each(function () {
			...
			if (b.recreateForm === true && a("#" + f.themodal).html() != null) {
				a("#" + f.themodal).remove();
			}
